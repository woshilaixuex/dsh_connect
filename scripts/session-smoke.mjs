 ...
 *
 * 用法:
 *   node scripts/session-smoke.mjs                       # 默认 ws://127.0.0.1:8080
 *   node scripts/session-smoke.mjs ws://192.168.1.10:8097
 *   SESSION_SMOKE_URL=ws://host:port node scripts/session-smoke.mjs
 *
 * 退出码:0 = 全部通过;1 = 有失败项。
 */

const argUrl = process.argv.find((a) => a.startsWith('ws://') || a.startsWith('wss://'))
const URL = argUrl ?? process.env.SESSION_SMOKE_URL ?? 'ws://127.0.0.1:8080'
...
async function main() {
  ...
  const firstAnswer = [...events].reverse().find((d) => d.kind === 'assistant.message')
  info('第一轮回复', JSON.stringify(firstAnswer?.text ?? '(无)'))
  ...
  const secondAnswer = [...events.slice(before)].reverse().find((d) => d.kind === 'assistant.message')
  const remembered = typeof secondAnswer?.text === 'string' && secondAnswer.text.includes('蓝鲸七号')
  info('第二轮回复', JSON.stringify(secondAnswer?.text ?? '(无)'))
  ...
  if (idleWaitMs > 0) {
    ...
    const resumedAnswer = [...events.slice(beforeResume)].reverse().find((d) => d.kind === 'assistant.message')
    info('resume 后回复', JSON.stringify(resumedAnswer?.text ?? '(无)'))
    const stillRemembered = typeof resumedAnswer?.text === 'string' && resumedAnswer.text.includes('蓝鲸七号')
